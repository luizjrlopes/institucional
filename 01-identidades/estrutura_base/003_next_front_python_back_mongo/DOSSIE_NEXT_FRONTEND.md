...
**Versão:** v1.0  
**Status:** Padrão Institucional (frontend com backend Python separado)
...
10. [Comunicação com Backend Python](#10-comunicação-com-backend-python)
...
Este dossiê define o padrão oficial de arquitetura de frontend para aplicações desenvolvidas em Next.js com App Router, quando o backend é uma aplicação Python (FastAPI) separada. Define contratos e boas práticas de comunicação entre as duas aplicações.
...
- Comunicação via API REST/HTTP com contratos tipados (Pydantic ↔ TypeScript)
...
src/
  ...
  components/
  features/
  services/        # Agentes que chamam a API externa (FastAPI)
  styles/
  hooks/
  store/
  utils/
...
## 10. Comunicação com Backend Python
...
- Padronizar DTOs e erros; quando possível usar contratos compartilhados (OpenAPI / JSON Schema)
...
- Preferir gerar tipos TypeScript a partir do OpenAPI/JSON Schema do FastAPI quando possível
...
Este dossiê adapta o padrão institucional do frontend ao cenário em que o backend é uma aplicação Python. Mantém princípios de separação de responsabilidade, tipagem e testes de contrato.
